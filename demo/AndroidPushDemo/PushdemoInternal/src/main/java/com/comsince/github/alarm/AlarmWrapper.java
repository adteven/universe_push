...
public class AlarmWrapper {
    ...
    public void start() {
        DebugLogger.i(tag, "start with " + mAuthority + " Android " + Build.VERSION.SDK_INT);
    ...
    public void stop() {
        DebugLogger.i(tag, "stop with " + mAuthority);
    ...
    public void schedule(final Timer timer) {
        DebugLogger.i(tag, "schedule " + timer);
    ...
    public void cancel(final Timer timer) {
        DebugLogger.i(tag, "cancel " + timer);
        if (timer != null && timer.key != 0) {
            Runnable task = new Runnable() {
                @Override
                public void run() {
    ...
    private class TimerReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, final Intent intent) {
            if (intent != null) {
                DebugLogger.i(tag, "on receive timer broadcast...");
    ...
    private class TimerListener implements
        ...
        @Override
        public void onAlarm() {
            DebugLogger.i(tag, "on alarm listener invoke...");
            if (Thread.currentThread().getId() == Schedules.event().threadId()) {
                run();
            } else {
