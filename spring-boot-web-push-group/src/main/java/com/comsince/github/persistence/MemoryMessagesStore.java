...
public class MemoryMessagesStore implements IMessagesStore {
    ...
    private MemorySessionStore memorySessionStore;

    public MemoryMessagesStore(DatabaseStore databaseStore,MemorySessionStore memorySessionStore,HazelcastInstance hazelcastInstance) {
    ...
    public WFCMessage.PullMessageResult fetchMessage(String user, String exceptClientId, long fromMessageId, int pullType) {
        ...
        if (pullType == ProtoConstants.PullType.Pull_ChatRoom) {
            chatroomId = (String)hzInstance.getMap(USER_CHATROOM).get(user);
        }
...
//                }
    ...
    public Collection<String> getChatroomMemberClient(String userId) {
        String chatroomId = (String)hzInstance.getMap(USER_CHATROOM).get(userId);
    ...
    public boolean checkUserClientInChatroom(String user, String clientId, String chatroomId) {
        ...
        String existChatroomId = (String)hzInstance.getMap(USER_CHATROOM).get(user);
...
//            databaseStore.updateUser(user);
    ...
    public int getChatroomMemberCount(String chatroomId) {
        return hzInstance.getMultiMap(CHATROOM_MEMBER_IDS).valueCount(chatroomId);
    }
    ...
    public ErrorCode handleJoinChatroom(String userId, String clientId, String chatroomId) {
        ...
        memorySessionStore.getSession(clientId).refreshLastChatroomActiveTime();
        hzInstance.getMap(USER_CHATROOM).put(userId, chatroomId);
        hzInstance.getMultiMap(CHATROOM_MEMBER_IDS).put(chatroomId, new UserClientEntry(userId, clientId));
    ...
    public ErrorCode handleQuitChatroom(String userId, String clientId, String chatroomId) {
        hzInstance.getMap(USER_CHATROOM).remove(userId);
        hzInstance.getMultiMap(CHATROOM_MEMBER_IDS).remove(chatroomId, new UserClientEntry(userId, clientId));
    ...
    public ErrorCode deleteFriend(String userId, String friendUid) {
        ...
        for (FriendData data : user1Friends) {
    ...
    public boolean checkUserInChannel(String user, String channelId) {
        MultiMap<String, String> chatroomMembers = hzInstance.getMultiMap(CHANNEL_LISTENERS);
        if (chatroomMembers == null) {
            return false;
        }
